...
class GEPAState:
    ...
    validation_api_calls: int
    minibatch_input_tokens: int
    minibatch_output_tokens: int
    minibatch_api_calls: int
    ...
    def __init__(
        ...
        self.validation_api_calls = 0
        self.minibatch_input_tokens = 0
        self.minibatch_output_tokens = 0
        self.minibatch_api_calls = 0
    ...
    def save(self, run_dir:str):
        for prog_idx, curr_prog in enumerate(self.program_candidates):
            ...
            try:
                ...
                save_format = "full_program"
            except Exception:
                # Some later-stage programs become too recursive for cloudpickle.
                # Fall back to DSPy state-only saving so checkpointing still succeeds.
                for leftover_name in ("program.pkl", "metadata.json", "program_state.pkl", "program_save_format.json"):
                    leftover_path = os.path.join(dir_to_save, leftover_name)
                    if os.path.exists(leftover_path):
                        os.remove(leftover_path)
                curr_prog.save(os.path.join(dir_to_save, "program_state.pkl"), save_program=False)
                save_format = "state_only"
            finally:
                curr_prog.set_lm(curr_prog_lm)
            with open(os.path.join(dir_to_save, "program_save_format.json"), "w", encoding="utf-8") as f:
                json.dump({"format": save_format}, f, indent=2)
    ...
    def load(run_dir: str) -> 'GEPAState':
        ...
        if not hasattr(state, 'validation_input_tokens'):
            setattr(state, 'validation_input_tokens', 0)
        if not hasattr(state, 'validation_output_tokens'):
            setattr(state, 'validation_output_tokens', 0)
        if not hasattr(state, 'validation_api_calls'):
            setattr(state, 'validation_api_calls', 0)
        if not hasattr(state, 'minibatch_input_tokens'):
            setattr(state, 'minibatch_input_tokens', 0)
        if not hasattr(state, 'minibatch_output_tokens'):
            setattr(state, 'minibatch_output_tokens', 0)
        if not hasattr(state, 'minibatch_api_calls'):
            setattr(state, 'minibatch_api_calls', 0)
        if not hasattr(state, 'combined_validation_full_scores'):
            setattr(state, 'combined_validation_full_scores', None)
        if not hasattr(state, 'combined_validation_val_subscores'):
            setattr(state, 'combined_validation_val_subscores', None)
        state.program_candidates = []
        expected_num_candidates = len(state.parent_program_for_candidate)
        for i in range(expected_num_candidates):
            dir_to_load = os.path.join(run_dir, "prog_candidates", str(i))
            if not os.path.exists(dir_to_load):
                raise FileNotFoundError(
                    f"Expected checkpoint directory for candidate {i} at {dir_to_load}, but it does not exist."
                )
            prog = GEPAState._load_program_candidate(dir_to_load, state.program_candidates)
    ...
    @staticmethod
    def _read_program_save_format(dir_to_load: str) -> str | None:
        format_path = os.path.join(dir_to_load, "program_save_format.json")
    ...
    @staticmethod
    def _load_program_candidate(dir_to_load: str, loaded_programs: list[dspy.Module]) -> dspy.Module:
        save_format = GEPAState._read_program_save_format(dir_to_load)
        full_program_path = os.path.join(dir_to_load, "program.pkl")
        full_metadata_path = os.path.join(dir_to_load, "metadata.json")
        state_only_path = os.path.join(dir_to_load, "program_state.pkl")

        if save_format in (None, "full_program") and os.path.exists(full_program_path) and os.path.exists(full_metadata_path):
            try:
                return dspy.load(dir_to_load, allow_pickle=True)
            except TypeError:
                # Some vendored DSPy revisions do not expose the allow_pickle kwarg.
                return dspy.load(dir_to_load)

        if os.path.exists(state_only_path):
            if not loaded_programs:
                raise RuntimeError(
                    f"Cannot reconstruct state-only checkpoint at {dir_to_load} because no earlier full-program "
                    "candidate is available as a template."
                )
            prog = loaded_programs[0].deepcopy()
            prog.load(state_only_path)
            return prog

        raise RuntimeError(
            f"Candidate checkpoint at {dir_to_load} is incomplete. "
            f"Expected either program.pkl+metadata.json or program_state.pkl."
        )
...
def make_selection_surrogate_eval_output(score: float, metadata: dict | None = None):
    payload = metadata or {"kind": "surrogate_score"}
...
def initialize_gepa_state(gepa_state_to_use, run_dir, logger, base_dspy_program, trainset_evaluator, valset_evaluator, seed, run_linearized_gepa, track_scores_on, train_val_size, selection_mode='validation'):
    if gepa_state_to_use is None:
        if os.path.exists(os.path.join(run_dir, "gepa_state.bin")) and os.path.exists(os.path.join(run_dir, "prog_candidates")):
            ...
            num_evals_run = 0
            validation_usage = {"input_tokens": 0, "output_tokens": 0, "api_calls": 0}
            combined_validation_eval_out = None
            if selection_mode == 'validation':
                ...
                active_lm = dspy.dsp.utils.settings.lm or base_dspy_program.get_lm()
                validation_history_start = len(getattr(active_lm, "history", [])) if active_lm is not None else 0
                valset_out = valset_evaluator(base_dspy_program)
                if active_lm is not None:
                    validation_traces = active_lm.history[validation_history_start:]
                    validation_usage = {
                        "input_tokens": sum((trace.get("usage", {}) or {}).get("prompt_tokens", 0) for trace in validation_traces),
                        "output_tokens": sum((trace.get("usage", {}) or {}).get("completion_tokens", 0) for trace in validation_traces),
                        "api_calls": len(validation_traces),
                    }
            ...
            elif selection_mode == 'validation_llm_judge_combined':
                assert track_scores_on == 'val', "selection_mode='validation_llm_judge_combined' currently requires track_scores_on='val'"
                eval_out = (None, [], [])

                active_lm = dspy.dsp.utils.settings.lm or base_dspy_program.get_lm()
                validation_history_start = len(getattr(active_lm, "history", [])) if active_lm is not None else 0
                combined_validation_eval_out = valset_evaluator(base_dspy_program)
                if active_lm is not None:
                    validation_traces = active_lm.history[validation_history_start:]
                    validation_usage = {
                        "input_tokens": sum((trace.get("usage", {}) or {}).get("prompt_tokens", 0) for trace in validation_traces),
                ...
                write_eval_output_to_directory(
                    combined_validation_eval_out,
                    os.path.join(run_dir, "generated_combined_validation_outputs_valset"),
                )
                num_evals_run += len(combined_validation_eval_out[2])
                valset_out = make_selection_surrogate_eval_output(
                    score=0.0,
                    metadata={
                        "selection_mode": selection_mode,
                        "kind": "base_program_combined_surrogate",
                        "raw_validation_score": combined_validation_eval_out[0],
                    },
                )
                write_eval_output_to_directory(valset_out, os.path.join(run_dir, "generated_best_outputs_valset"))
            else:
                assert selection_mode in {'llm_judge', 'llm_judge_score_aware'}, f"Unknown selection mode {selection_mode}"
                assert track_scores_on == 'val', "selection_mode='llm_judge' currently requires track_scores_on='val'"
                eval_out = (None, [], [])
                valset_out = make_selection_surrogate_eval_output(
                    score=0.0,
                    metadata={"selection_mode": selection_mode, "kind": "base_program_surrogate"},
                )
                write_eval_output_to_directory(valset_out, os.path.join(run_dir, "generated_best_outputs_valset"))
            ...
            if selection_mode == 'validation_llm_judge_combined':
                gepa_state.combined_validation_full_scores = [combined_validation_eval_out[0]]
                gepa_state.combined_validation_val_subscores = [list(combined_validation_eval_out[2])]

            gepa_state.num_full_ds_evals = 1 if selection_mode in {'validation', 'validation_llm_judge_combined'} else 0
            gepa_state.total_num_evals_per_trainval_instance = num_evals_run / train_val_size if num_evals_run > 0 else 0
            gepa_state.total_num_evals = num_evals_run
            gepa_state.validation_input_tokens = validation_usage["input_tokens"]
            gepa_state.validation_output_tokens = validation_usage["output_tokens"]
            gepa_state.validation_api_calls = validation_usage["api_calls"]
...
def log_detailed_metrics_after_discovering_new_program(logger, gepa_state, valset_score, new_prog_all_scores, full_score, new_program_idx, valset_subscores, new_instruction, use_wandb, linear_pareto_front_program_idx, track_scores_on, selection_mode='validation', selection_metadata=None):
    ...
    if selection_mode == 'validation':
        ...
        logger.log(f"Iteration {gepa_state.i+1}: Selection surrogate score for new program: {valset_score}")
        logger.log(f"Iteration {gepa_state.i+1}: Surrogate scores for new program: {valset_subscores}")
        logger.log(f"Iteration {gepa_state.i+1}: Surrogate score frontier: {gepa_state.pareto_front_valset}")
        logger.log(f"Iteration {gepa_state.i+1}: Updated surrogate frontier programs: {gepa_state.program_at_pareto_front_valset}")
        logger.log(f"Iteration {gepa_state.i+1}: Best surrogate score so far: {max(gepa_state.program_full_scores_val_set)}")
        logger.log(f"Iteration {gepa_state.i+1}: Best program as per surrogate score: {best_prog_as_per_agg_score_valset}")
        if selection_metadata is not None:
            logger.log(f"Iteration {gepa_state.i+1}: Judge selection metadata: {selection_metadata}")
    ...
    if selection_mode == 'validation':
        ...
        wandb_logs = {
            "iteration": gepa_state.i+1,
            "selection_surrogate_score": valset_score,
            "new_instruction": new_instruction,
            "new_program_idx": new_program_idx,
            "surrogate_front_scores": gepa_state.pareto_front_valset,
            "surrogate_score_new_program": valset_subscores,
            "best_surrogate_score": max(gepa_state.program_full_scores_val_set),
            "linear_pareto_front_program_idx": linear_pareto_front_program_idx,
            "best_program_as_per_surrogate_score": best_prog_as_per_agg_score_valset,
        }
        if selection_metadata is not None:
            wandb_logs.update({
                "judge_confidence": selection_metadata.get("confidence"),
                "judge_parse_status": selection_metadata.get("parse_status"),
            })
