...
def call_lm_and_extract_response(prompt, lm, current_instruction_doc, user_examples_and_feedback, reference_materials=None):
    ...
    # Reuse the LM-configured completion budget instead of hard-coding a larger one
    # that may exceed the active provider's gateway limit.
    lm_kwargs = getattr(lm, "kwargs", {}) or {}
    if "max_completion_tokens" in lm_kwargs:
        completion_key = "max_completion_tokens"
    else:
        completion_key = "max_tokens"
    completion_budget = lm_kwargs.get(completion_key) or 4096

    lm_out = lm(full_prompt, **{completion_key: completion_budget})[0].strip()
    # Extract ``` blocks
    if lm_out.count("```") >= 2:
        start = lm_out.find("```")
        end = lm_out.rfind("```")
